...
- **单文件三段（PRD → Spec → Plan）**：一份 `.biaospec/<日期>-<主题>/spec.md` 从头到尾，任务各有独立目录，与项目 `docs/` 分开、不互相污染。
...
```
/plugin marketplace add kyriewang/biao-spec
/plugin install biao-spec@biao-spec
```

装好后两个技能在所有项目可用，在 customize 面板里可开关。本地开发时把 `kyriewang/biao-spec` 换成本仓库的本地路径即可。

### 方式二：手动放入技能目录

技能就是「一个含 `SKILL.md` 的文件夹」，把 `skills/` 下两个目录放到平台技能目录即可：

- **Claude Code 用户级**（所有项目可用）：软链或复制到 `~/.claude/skills/`
- **Claude Code 项目级**（随 git 同步给团队）：放到 `<项目>/.claude/skills/`
- **codex**：放到 `<项目>/.agents/skills/`

```sh
# 软链方式：改动即时同步，适合自己迭代
ln -s "$PWD/skills/biaospec-brainstorm" ~/.claude/skills/biaospec-brainstorm
ln -s "$PWD/skills/biaospec-plan" ~/.claude/skills/biaospec-plan
```

> Claude Code 只从 `.claude/skills/` 加载技能，`.agents/skills/` 是 codex 的约定；两平台共用同一份 `SKILL.md`。
...
2. 收敛后把设计直接写成 `.biaospec/<日期>-<主题>/spec.md`（状态=需求澄清中），你在文件上评审；
